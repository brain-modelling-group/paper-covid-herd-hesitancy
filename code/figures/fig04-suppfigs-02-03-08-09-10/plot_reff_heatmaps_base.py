...
def get_heatmap_df(vax_covs, vax_effs, args):
    ...
    for idx1, vax_eff in enumerate(vax_effs):
        for idx2, vax_cov in enumerate(vax_covs):
            data = pd.read_csv(f'..{sep}../case1a{sep}results{sep}results-{args.variant}-{args.age_lb}{sep}sim-data-vax{sep}summary-stats{sep}qld_simple-vax_apply-tt_0020_iqf_0.00_vxstr_none_vxcov_{vax_cov:.{2}f}_vxeff_{vax_eff:.{2}f}_age-lb_{args.age_lb}.csv')
